...
def main():
    ...
    # --- Фильтр: скоро дедлайн (0..THRESHOLD_DAYS) ---
    near = []
    for task in tasks:
    ...
    if not near:
        cmf_alert('Задач с приближающимся дедлайном нет')
        return
    ...
    for email, data in by_email.items():
        content = f'<h2>СКОРО ДЕДЛАЙН — {data["name"]}</h2><hr>'
        content += f'Порог: <= {THRESHOLD_DAYS} дн.<br><br>'
        content += format_task_list(data['tasks'])
        subject = f'Скоро дедлайн: задач = {len(data["tasks"])}'
