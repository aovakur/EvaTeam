...
def main():
    ...
    for task in tasks:
        ...
        if finish and (finish - today).days < 0:
            overdue.append(task)

    if not overdue:
        cmf_alert('Просроченных задач нет')
        return
    ...
    for email, data in by_email.items():
        content = f'<h2>ПРОСРОЧЕННЫЕ ЗАДАЧИ — {data["name"]}</h2><hr>'
        content += format_task_list(data['tasks'])
        subject = f'Просрочено задач: {len(data["tasks"])}'
